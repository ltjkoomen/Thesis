...
class RNN(object):
    ...
    def __init__(self, input, n_in, n_hidden, n_out, activation=T.tanh,
        ...
        self.dt   = 0.1
        self.SS_A = np.array([[1, self.dt],[0, 1]])
        self.SS_B = np.array([[0.5*self.dt**2],[self.dt]])
        
        x0_init = np.zeros((self.SS_A.shape[0],), dtype=theano.config.floatX)
        self.SS_x0 = theano.shared(value=x0_init, name='SS_x0')
        
        y0_init = np.zeros((1,), dtype=theano.config.floatX)
        self.SS_y0 = theano.shared(value=x0_init, name='SS_y0')
        
        # recurrent function (using tanh activation function) and linear output
        # activation function
        def step(SS_ym1, h_tm1, SS_x):
            
            # RNN step
            h_t = self.activation(T.dot(SS_ym1, self.W_in) + \
                                  T.dot(h_tm1, self.W) + self.bh)
            y_t = T.dot(h_t, self.W_out) + self.by
            
            # SS step
            SS_x = T.dot(SS_x, self.SS_A) + T.dot(y_t, self.SS_B)
            SS_y = SS_x[1]/SS_x[0]
            
            return h_t,SS_x,SS_y

        # the hidden state `h` for the entire sequence, and the output for the
        # entire sequence `y` (first dimension is always time)
        [self.h, self.SS_x, self.y_pred], _ = theano.scan(step,
                                               sequences=self.input,
                                               outputs_info=[self.h0, self.SS_x0, self.SS_y0])
